...
void main() {
  group('wallet_test:', () {
    ...
    test('Wallet convert works properly.', () {
      ...
      final wallet = Wallet.derive(
          mnemonic: mnemonic.split(' '), networkInfo: bluzelleInfo);
      ...
      final converted =
          Wallet.convert(wallet: wallet, networkInfo: godzelleInfo);
    ...
    test('Wallet sign generates deterministic signatures.', () {
      ...
      final data = 'Test';
      final sig1 =
          hex.encode(wallet.sign(Uint8List.fromList(utf8.encode(data))));
      final sig2 =
          hex.encode(wallet.sign(Uint8List.fromList(utf8.encode(data))));
